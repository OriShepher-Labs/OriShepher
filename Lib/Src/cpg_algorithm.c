...
//const：常量，初始化后不能修改。
//double M_PI = 3.14159265358979323846; 由于<math.h>头文件提供了标准的M_PI宏定义,无需将M_PI定义为一个变量

/**
 * @brief 初始化CPG状态结构体
 * @param state [输入/输出] 指向CPG_State结构体的指针，函数将对其成员进行初始化。
 ...
 * @param state [输入/输出] 指向CPG_State结构体的指针，函数将修改其phase成员。
...
void CPG_setFrequency(CPG_State *state, double freq, double speed_multiplier) {
    // 遍历3个振荡器，设置频率为freq并限制在(-5.5, 5.5)范围内
    for (int i = 0; i < 3; i++) {
        state->omega[i] = M_PI * fabs(OF_mapToRange(-5.5, 5.5, freq) * speed_multiplier); // freq为Hz
    }
    // freq为负时调转相位为后退状态
    if (freq < 0) {
 ...
 * @param state [输入] 指向CPG_State结构体的指针，提供当前的状态变量和参数。
...
/**
 * @brief 欧拉法更新CPG状态
 * @param state [输入/输出] 指向CPG_State结构体的指针，函数将根据微分方程更新其状态变量和时间。
